...
//    private Cryptography.Aes GetAlgorithm()
//    {
//        var result = Cryptography.Aes.Create();
//        result.KeySize = KeySize;
//        result.BlockSize = BlockSize;
//        result.Padding = Cryptography.PaddingMode.ISO10126;
//        result.Mode = Cryptography.CipherMode.CBC;
//        return result;
//    }
...
//        var iv = Token.Create(IvSize); // 32B
//        var plainText = input;
//        using (var deriveBytes = new Cryptography.Rfc2898DeriveBytes(key, salt, 1000, Cryptography.HashAlgorithmName.SHA256))
//        {
//            var keyBytes = deriveBytes.GetBytes(IvSize); // 32B
//            using var algorithm = GetAlgorithm();
//            using (var encryptor = algorithm.CreateEncryptor(keyBytes, iv))
//            {
//                using (var memory = new MemoryStream())
//                {
//                    using (var cryptoStream = new Cryptography.CryptoStream(memory, encryptor, Cryptography.CryptoStreamMode.Write))
//                    {
//                        cryptoStream.Write(plainText, 0, plainText.Length);
//                        cryptoStream.FlushFinalBlock();
//                        var bytes = salt;
...
//        var salt = input.Take(IvSize).ToArray();
//        var iv = input.Skip(IvSize).Take(IvSize).ToArray();
//        var cipherCodeBytes = input.Skip(IvSize * 2).Take(input.Length - (2 * IvSize)).ToArray();
//        using (var deriveBytes = new Cryptography.Rfc2898DeriveBytes(key, salt, 1000, Cryptography.HashAlgorithmName.SHA256))
//        {
//            var keyBytes = deriveBytes.GetBytes(IvSize);
//            using var algorithm = GetAlgorithm();
//            using var decryptor = algorithm.CreateDecryptor(keyBytes, iv);
//            using (var memory = new MemoryStream(cipherCodeBytes))
//            {
//                using (var cryptoStream = new Cryptography.CryptoStream(memory, decryptor, Cryptography.CryptoStreamMode.Read))
//                {
//                    using (var stream = new MemoryStream())
//                    {
//                        cryptoStream.CopyTo(stream);
